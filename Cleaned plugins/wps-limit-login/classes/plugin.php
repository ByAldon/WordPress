...
class Plugin {
	...
	public $default_options = array(
		...
		'wps_limit_login_show_credit_link' => false,
	);
	/**
	...
	/**
	 * Sanitize a positive integer and keep it inside a safe range.
	 */
	private static function sanitize_limited_int( $value, $default, $min, $max ) {
		$value = absint( $value );
	...
	/**
	 * Sanitize an IP/range textarea while keeping supported range syntax.
	 * Supported input remains compatible with the original plugin: exact IPs,
	 * IPv4 wildcards, CIDR-like ranges and dash ranges, one entry per line.
	 */
	...
	public function sanitize_options() {
		...
		if ( isset( $_POST['lockout_notify_email'] ) ) {
			$notify = sanitize_text_field( wp_unslash( $_POST['lockout_notify_email'] ) );
			$this->update_option( 'wps_limit_lockout_notify', 'email' === $notify ? 'email' : '' );
		} else {
	...
	public function get_address( $type_name = '' ) {
		...
		return sanitize_text_field( (string) $type_name );
	}
	...
	public function options_page() {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have permission to access this page.', 'wps-limit-login' ) );
		}
		...
		if ( ! empty( $_POST ) ) {
			...
			if ( '' === $tab ) {
				...
				if ( isset( $_POST['update_options'] ) ) {
					$allowed_retries  = isset( $_POST['allowed_retries'] ) ? self::sanitize_limited_int( wp_unslash( $_POST['allowed_retries'] ), 3, 1, 20 ) : 3;
					$lockout_minutes  = isset( $_POST['lockout_duration'] ) ? self::sanitize_limited_int( wp_unslash( $_POST['lockout_duration'] ), 20, 1, 1440 ) : 20;
					$allowed_lockouts = isset( $_POST['allowed_lockouts'] ) ? self::sanitize_limited_int( wp_unslash( $_POST['allowed_lockouts'] ), 2, 1, 20 ) : 2;
					$long_hours       = isset( $_POST['long_duration'] ) ? self::sanitize_limited_int( wp_unslash( $_POST['long_duration'] ), 24, 1, 168 ) : 24;
					$valid_hours      = isset( $_POST['valid_duration'] ) ? self::sanitize_limited_int( wp_unslash( $_POST['valid_duration'] ), 12, 1, 168 ) : 12;
					$notify_after     = isset( $_POST['notify_email_after'] ) ? self::sanitize_limited_int( wp_unslash( $_POST['notify_email_after'] ), 2, 1, 20 ) : 2;

					$this->update_option( 'wps_limit_login_allowed_retries', $allowed_retries );
					$this->update_option( 'wps_limit_login_lockout_duration', $lockout_minutes * 60 );
					$this->update_option( 'wps_limit_login_allowed_lockouts', $allowed_lockouts );
					$this->update_option( 'wps_limit_login_long_duration', $long_hours * 3600 );
					$this->update_option( 'wps_limit_login_valid_duration', $valid_hours * 3600 );
					$this->update_option( 'wps_limit_login_notify_email_after', $notify_after );
					$this->update_option( 'wps_limit_login_show_credit_link', ! empty( $_POST['show_credit_link'] ) ? 'true' : '' );
				...
				if ( isset( $_POST['update_options'] ) ) {
					$whitelist = isset( $_POST['wps_limit_login_whitelist_ips'] ) ? self::sanitize_ip_list( $_POST['wps_limit_login_whitelist_ips'] ) : array();
					$this->update_option( 'wps_limit_login_whitelist', $whitelist );
					self::show_error( __( 'Options saved.', 'wps-limit-login' ) );
				}
			} elseif ( 'blacklist' === $tab ) {
				if ( isset( $_POST['update_options'] ) ) {
					$blacklist = isset( $_POST['wps_limit_login_blacklist_ips'] ) ? self::sanitize_ip_list( $_POST['wps_limit_login_blacklist_ips'] ) : array();
					$this->update_option( 'wps_limit_login_blacklist', $blacklist );
					self::show_error( __( 'Options saved.', 'wps-limit-login' ) );
				}
			}
		}
	...
	public function ajax_unlock() {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( array( 'message' => __( 'Permission denied.', 'wps-limit-login' ) ), 403 );
		}
		...
		if ( is_array( $log ) && isset( $log[ $ip ][ $user_login ] ) ) {
			if ( ! is_array( $log[ $ip ][ $user_login ] ) ) {
				$log[ $ip ][ $user_login ] = array(
					'counter' => absint( $log[ $ip ][ $user_login ] ),
				);
			}
	...
	public static function show_error( $msg ) {
		...
		echo '<div id="message" class="updated fade"><p>' . wp_kses_post( $msg ) . '</p></div>';
	}
	...
	public function login_form() {
		// ByAldon custom build: keep the login screen clean and do not output credit/promotional links.
		return false;
	}
	...
	public function reinitialize() {
		...
		if ( ! wp_verify_nonce( $nonce, 'reinitialize' ) ) {
			return false;
		}

		$action = isset( $_GET['action'] ) ? sanitize_key( wp_unslash( $_GET['action'] ) ) : '';
		if ( 'reinitialize' !== $action ) {
			return false;
		}
		...
		wp_safe_redirect( $this->get_wps_limit_login_options_page_uri() );
		exit;
	}
	...
	public function plugin_action_links( $links ) {
		if ( is_network_admin() && is_plugin_active_for_network( WPS_LIMIT_LOGIN_BASENAME ) ) {
			array_unshift( $links, '<a href="' . esc_url( network_admin_url( 'settings.php?page=wps-limit-login' ) ) . '">' . esc_html__( 'Settings' ) . '</a>' );
		} else {
			array_unshift( $links, '<a href="' . esc_url( admin_url( 'options-general.php?page=wps-limit-login' ) ) . '">' . esc_html__( 'Settings' ) . '</a>' );
		}
	...
	public function dashboard_widget_function( $post, $callback_args ) {
		...
		foreach ( $log as $date => $user_info ) :
			...
			echo '<p>' . esc_html( date_i18n( 'd/m/Y H:i:s', $date ) ) . ' - ' . esc_html( $user_info['ip'] ) . ' - ' . esc_html( $user_info['username'] ) . ' (' . absint( $user_info['counter'] ) . ' ' . esc_html( _n( 'lockout', 'lockouts', absint( $user_info['counter'] ), 'wps-limit-login' ) ) . ')' . '</p>';
			$i ++;
		endforeach; ?>
		...
		echo '<a class="button button-primary" href="' . esc_url( admin_url( 'options-general.php?page=wps-limit-login&tab=log' ) ) . '">' . esc_html__( 'See all lockouts', 'wps-limit-login' ) . '</a>';
