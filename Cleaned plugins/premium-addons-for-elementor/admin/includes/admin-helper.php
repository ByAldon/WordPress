...
class Admin_Helper {
	...
	public function __construct() {
		...
		// ByAldon custom hardening: newsletter subscription AJAX disabled in this cleaned build.
		...
		if ( is_admin() ) {
			...
			// ByAldon custom hardening: deactivation feedback/telemetry popup disabled.
		}
	}
	...
	public function insert_action_links( $links ) {
		...
		$settings_link = sprintf( '<a href="%1$s">%2$s</a>', esc_url( admin_url( 'admin.php?page=' . self::$page_slug . '#tab=elements' ) ), esc_html__( 'Settings', 'premium-addons-for-elementor' ) );
	...
	public function plugin_row_meta( $meta, $file ) {
		...
		// ByAldon custom hardening: extra promotional row-meta links disabled.
		return $meta;
	}
	...
	public function add_menu_tabs() {
		...
		// ByAldon custom hardening: PRO upsell submenu disabled.
	...
	public function run_pa_rollback() {
		...
		wp_die(
			esc_html__( 'Rollback is disabled in this cleaned update-locked build. Upload a reviewed ZIP manually if you want to replace it.', 'premium-addons-for-elementor' ),
			esc_html__( 'Updates locked', 'premium-addons-for-elementor' ),
			array( 'response' => 403 )
		);
	}
	...
	public function subscribe_newsletter() {
		...
		wp_send_json_error( array( 'message' => __( 'Newsletter subscription is disabled in this cleaned build.', 'premium-addons-for-elementor' ) ) );
	}
	...
	public function get_pa_news() {
		...
		// ByAldon custom hardening: remote dashboard news disabled.
		return array();
	}
