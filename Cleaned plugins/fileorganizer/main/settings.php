...
function fileorganizer_page_header($title = 'FileOrganizer'){
	...
	<div class="fileorganizer-box-container" style="margin:0">
		<table class="fileorganizer-settings-header" cellpadding="2" cellspacing="1" width="100%" class="fixed" border="0">
			...
			if(!defined('SITEPAD')){
				echo '<td align="right"><a target="_blank" rel="noopener noreferrer" class="button button-primary" href="https://wordpress.org/support/view/plugin-reviews/fileorganizer">Review FileOrganizer</a></td>';
			}
			echo '<td align="right" width="40"><a target="_blank" rel="noopener noreferrer" href="https://twitter.com/fileorganizer"><img src="'.esc_url(FILEORGANIZER_URL).'/images/twitter.png" /></a></td>
				<td align="right" width="40"><a target="_blank" rel="noopener noreferrer" href="https://www.facebook.com/fileorganizer/"><img src="'.esc_url(FILEORGANIZER_URL).'/images/facebook.png" /></a></td>
			</tr>
		</table>
	</div>';
		...
		<div class="fileorganizer-promotion" style="width:100%;" >
			<div class="fileorganizer-promotion-content">
				<h2 class="fileorganizer-promotion-logo">
					<span><a target="_blank" rel="noopener noreferrer" href="https://pagelayer.com/?from=fileorganizer-plugin"><img src="'. esc_url(FILEORGANIZER_URL).'/images/pagelayer_product.png" width="100%"></a></span>
				</h2>
				<div>
					...
					<center><a class="button button-primary" target="_blank" rel="noopener noreferrer" href="https://pagelayer.com/?from=fileorganizer-plugin">Visit Pagelayer</a></center>
				</div>
			</div>

			<div class="fileorganizer-promotion-content">
				<h2 class="fileorganizer-promotion-logo">
					<span><a target="_blank" rel="noopener noreferrer" href="https://loginizer.com/?from=fileorganizer-plugin"><img src="'.esc_url(FILEORGANIZER_URL).'/images/loginizer_product.png" width="100%"></a></span>
				</h2>
				<div>
					...
					<center><a class="button button-primary" target="_blank" rel="noopener noreferrer" href="https://loginizer.com/?from=fileorganizer-plugin">Visit Loginizer</a></center>
				</div>
			</div>
		</div>';
...
function fileorganizer_settings_page(){
	...
	if(isset($_POST['save_settings'])){
		...
		$options['default_view'] = fileorganizer_sanitize_default_view(fileorganizer_optpost('default_view', 'list'));
		$options['default_lang'] = sanitize_file_name(fileorganizer_optpost('default_lang', 'en'));
		$options['hide_htaccess'] = fileorganizer_optpost('hide_htaccess') === 'yes' ? 'yes' : '';
		$options['enable_trash'] = fileorganizer_optpost('enable_trash') === 'yes' ? 'yes' : '';

		if(defined('FILEORGANIZER_PRO')){
			$posted_roles = fileorganizer_optpost('user_roles', array());
				$allowed_roles = array('administrator', 'editor', 'author', 'contributor', 'subscriber', 'customer', 'shop_manager');
				$options['user_roles'] = is_array($posted_roles) ? array_values(array_intersect($allowed_roles, $posted_roles)) : array();
			$options['disable_path_restriction'] = fileorganizer_optpost('disable_path_restriction') === 'yes' ? 'yes' : '';
			$options['max_upload_size'] = max(0, absint(fileorganizer_optpost('max_upload_size')));
			$options['enable_ftp'] = fileorganizer_optpost('enable_ftp') === 'yes' ? 'yes' : '';
		}
