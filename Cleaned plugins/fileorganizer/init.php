...
function fileorganizer_load_plugin(){
	...
	if(false && !defined('SITEPAD') && is_admin() && !defined('FILEORGANIZER_PRO') && current_user_can('activate_plugins')){
		// The promo time
	...
	if(false && !defined('SITEPAD') && is_admin() && current_user_can('manage_options')){
...
function fileorganizer_page_handler(){
	...
	$theme = !empty($fileorganizer->options['theme']) ? fileorganizer_sanitize_theme($fileorganizer->options['theme']) : '';
...
function fileorganizer_allowed_themes(){
	return array('', 'dark', 'material', 'material-dark', 'material-gray', 'windows10');
}

function fileorganizer_sanitize_theme($theme){
	$theme = sanitize_key((string) $theme);

	if(!in_array($theme, fileorganizer_allowed_themes(), true)){
		return '';
	}
...
function fileorganizer_sanitize_default_view($view){
	$view = sanitize_key((string) $view);
	return in_array($view, array('icons', 'list'), true) ? $view : 'list';
}
...
function fileorganizer_validate_path($path) {
	...
	$currentDirectory = realpath($fileorganizer->default_path);
	$absolutePath = realpath($path);

	if(empty($currentDirectory) || empty($absolutePath)){
		return false;
	}

	$currentDirectory = fileorganizer_cleanpath($currentDirectory);
	$absolutePath = fileorganizer_cleanpath($absolutePath);
	
	if($currentDirectory === $absolutePath){
		return true;
	}

	return strpos($absolutePath . '/', $currentDirectory . '/') === 0;
}